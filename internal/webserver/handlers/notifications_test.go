...
func TestHandlePostGetAndDelete(t *testing.T){
	...
	req, err := http.NewRequest(http.MethodPost, server.URL, bytes.NewReader(webhookJSON))
	if err != nil {
		t.Fatal(err)
		return 
	}
	...
	getRequest, err := http.Get(server.URL + "/" + registration.ID)
	if err != nil{
    ...
    delRequest, delError := http.NewRequest(http.MethodDelete, server.URL + "/" + registration.ID, nil)
    if delError != nil {
	...
	newGetRequest, err := http.Get(server.URL + "/" + registration.ID)
	if err != nil{
		t.Fatal("Error on doing the get request")
		return 
	}
